...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen bg-primary flex items-center justify-center px-[5vw] pt-[10vh]">
      <div className="flex flex-col lg:flex-row items-center gap-[clamp(2rem,5vw,5rem)]">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}
          ...
          <div className="text-center mb-[3vh]">
            ...
            <h1 className="font-poppins font-extrabold text-[clamp(1.5rem,2vw,2rem)] text-white">Welcome Back</h1>
            <p className="font-inter text-[clamp(0.8rem,0.9vw,0.95rem)] text-text-muted mt-[0.5vh]">Login to your account</p>
          </div>

          <form onSubmit={handleSubmit} className="flex flex-col gap-[2vh]">
            <div>
              <label className="font-inter text-[clamp(0.75rem,0.8vw,0.85rem)] text-text-muted mb-1 block">Email</label>
              <input 
                type="email" 
                required 
                value={form.email} 
            ...
            <button 
              ...
              className="w-full font-inter font-semibold text-[clamp(0.9rem,1vw,1.05rem)] text-white bg-accent py-[clamp(0.7rem,1vh,1rem)] rounded-lg hover:bg-accent-dark hover:shadow-[0_0_25px_rgba(255,140,50,0.3)] transition-all duration-300 disabled:opacity-50 mt-2"
            >
              {loading ? 'Logging in...' : 'Login'}
            </button>
          </form>
          <p className="font-inter text-[clamp(0.8rem,0.85vw,0.9rem)] text-text-muted text-center mt-[2vh]">
            Don&apos;t have an account? <Link href="/register" className="text-accent hover:text-accent-dark transition-colors">Register</Link>
          </p>
        </motion.div>
      </div>
    </div>
